...
- [~] **1. Evidencia antes/después visible** *(visual · bajo)* — HECHO (23/08) el grueso: galería "La evidencia" en la historia del incidente (antes/durante/después, cada foto con sello de fecha + coordenadas) y comparador **foto | Street View** al seleccionar un punto en el mapa. Street View degrada solo: embebido con `NEXT_PUBLIC_GOOGLE_MAPS_KEY`, enlace si no hay clave. **Ojo: hay 0 fotos cargadas** — el circuito se verificó con fotos temporales; se llena cuando las cuadrillas empiecen a subir desde /campo. *(Falta: miniaturas en Campo, fotos dentro del reporte PDF, y el comparador deslizable antes/después.)*
  - ⚠ Deuda detectada de paso: `tomada_en` guarda el `now()` del servidor, no la hora real de la captura (EXIF) — para que la foto sea prueba irrefutable el sello debería ser el de la toma. Y `fotografias` no tiene policy de UPDATE/DELETE: una foto mal subida es inmutable desde la app.

- [x] **2. Distritos reales + métricas por distrito** *(datos+gestión · bajo)* — COMPLETO (23/08): los 20 distritos cargados (migración 0004) + backfill espacial (2.781/2.820 demandas, 1.792/1.824 incidentes) + `brechaPorDistrito()` + ranking clickeable en /brecha + coropleta de deuda en el mapa + aislamiento `?distrito=N` + herramienta `brecha_por_distrito` en Migue. Hallazgo político: **Distrito 20 con 100 % sin respuesta y cero m²**; el 17 con 96 %; seis distritos sin un solo m² ejecutado. *(Queda suelto: reemplazar el select provisorio de Funcionarios, que hoy usa nombres que no coinciden con los 20 oficiales.)*
...
- [ ] **20. Salida de beta: FORCE RLS + acceso cerrado + visor de auditoría** *(gestión · alto)* — ⚠ Incluir: la ingesta (CLI, web y GPKG) corre por fuera del RLS y upsertea por idRemoto que aporta el cliente — al endurecer, rol de sistema para ingesta + validar pertenencia/estado de los idRemoto antes de actualizar (hoy cualquier rol habilitado podría pisar demandas del consolidado re-importando). — Checklist D8: rol de aplicación no-dueño + FORCE ROW LEVEL SECURITY (hoy la app bypasea sus propias políticas), WITH CHECK para HCD, rol de sistema para ingesta; activar `DEV_SSO_CODIGO` (es setear una env) y alta de CIMBA como sistema externo en Ciudad Digital (el SSO ya está programado punta a punta). Más `/auditoria`: la tabla se llena sola por triggers desde el día cero y no tiene UI. **EL prerrequisito para que la Intendencia lo adopte.**
