...
- RIESGO PREVENTIVO (capa del mapa): puntúa 0-100 cada cuadra pavimentada por riesgo de romperse — 40 reincidencia (pedidos posteriores a la última reparación), 25 presión abierta, 20 antigüedad, 15 historial de reparaciones. Muestra solo las de 25+ (unas ~600). Es la base del bacheo preventivo.
- EL PULSO (/pulso): el parte diario que el sistema manda solo a las 7:00 por push y email (configurable en el tablero de avisos): qué entró ayer, qué se reparó y quién, vecinos respondidos, deuda de bacheo hoy vs hace 7 días, órdenes que vencen, cierres pendientes y la anomalía del día (barrio con 3× su promedio).
- ACTIVIDAD (/actividad, solo admin y planificación): quién usa el sistema y quién no (ingresos y acciones de los últimos 30 días por persona) y el feed de movimientos — sale de la auditoría automática de la base, no se puede editar ni borrar desde la app.
- PRODUCTIVIDAD (/ordenes/productividad): baches por día y por mes, por ejecutor, con m² y m³, siempre sobre intervenciones FINALIZADAS; compara el ritmo real contra la vara de capacidad.
...
- CUADRILLAS PROPIAS (unificación Campo ↔ Órdenes): la Administración es un EJECUTOR más — el Director le emite órdenes de trabajo a "ADMINISTRACIÓN (cuadrillas propias)" igual que a una contratista, y el capataz de la cuadrilla (rol cuadrilla) las reporta en el mismo portal /empresa con medidas + foto + tipo de intervención. /campo queda para las intervenciones programadas directas (el flujo viejo).
...
- CORRECCIÓN DE PINES EN LOTE (/calidad/pines): los pedidos con ubicación dudosa (<75%) no se trabajan solos; la IA geocodifica tandas de 20 y propone solo lo que clava con altura/esquina exacta — una persona confirma. Es el paso previo a RELEVAR EL CIRCUITO (botón en Nueva orden) que convierte los rojos limpios del circuito en incidentes priorizados agrupados a 25 m.
- TRATAMIENTO DE LA DEMANDA (/calidad): el sistema cruza cada reclamo abierto contra la red vial, las reparaciones y los demás reclamos, y hace saltar señales automáticas: "no es bache" (cae en ripio → se deriva a Ingeniería), "posible duplicada" (otro igual a <15 m), "parece ya resuelta" (reparación posterior a <25 m) y "para la SAT". El sistema propone, una persona confirma caso por caso desde las bandejas de /calidad/tratamiento.
- NOTA A LA SAT (/expedientes): las demandas de agua NO se derivan sueltas — se genera una nota administrativa numerada (NOTA-SAT-año-nnnn) dirigida al Director de la S.A.T. Dr. Marcelo Caponio, con georreferencia y foto de cada reclamo. La nota queda registrada e inmutable en /expedientes (detalle congelado, reimprimible) y recién ahí los reclamos salen de la cola de bacheo con la referencia del expediente.
